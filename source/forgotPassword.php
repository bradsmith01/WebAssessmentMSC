...
//The File with the database connection
require_once "../includes/config.php";



// Set sthe variables to be used equal to nothing EG "".
$new_password = $confirm_password = "";
$new_password_err = $confirm_password_err = "";
 
//Process the input data from reset password Submit  button Request
if($_SERVER["REQUEST_METHOD"] == "POST"){
 
    // Checks the password is longer than 6 characters or that the input isnt epty
    ...
    // Check that the confirm password is equal to the other password input
    //FYI the confirmed password does not get input intot the database.
    //ITs so the user can double check its the correct password they typed.
    if(empty(trim($_POST["confirm_password"]))){
        $confirm_password_err = "Please confirm the password.";
    } else{
      //Sends to the error message
    ...
    //Check input errors before sending SQL Statemtn to updat the users hashed password
    if(empty($new_password_err) && empty($confirm_password_err)){
        //Prepare an update statement
        $sql = "UPDATE users SET password = ? WHERE id = ?";
        
        if($stmt = mysqli_prepare($conn, $sql)){
            //Bind variables to the prepared statement as parameters so avoid SQL CODE IN the input fields
            mysqli_stmt_bind_param($stmt, "si", $param_password, $param_id);
            
            //PAssword is hashed as a form of encryption. If some hacker gets access to the
            //database then they cant ead the persons password directly.
            $param_password = password_hash($new_password, PASSWORD_DEFAULT);
            $param_id = $_SESSION["id"];
            
            //Attempt to execute the prepared statement
            if(mysqli_stmt_execute($stmt)){
                //Password will execute then distroy the current session and redirect user to the login page
                session_destroy();
                header("location: login.php");
                exit();
            } else{
              //Catch net else statement 
                echo "Error Occured Please try again.";
            }

            //Close statement in the DB Connection
...
<html lang="en">
    <head>
        ...
        <!--Title of my page-->
        <title>BAYKOST &dash; Reset Password</title>
        ...
        <div class="container text-light">
          ...
          <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post"> 
            <div class="form-group">
              ...
              <input type="password" name="new_password" class="form-control <?php echo (!empty($new_password_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $newPassword; ?>">
            ...
            <div class="form-group">
                ...
                <a class="btn btn-link ml-2" href="../source/accountDemo.php">Cancel</a>
            </div>
        </form>
    </div>  
