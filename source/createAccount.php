...
//Initialize with empty values so they can be not shown until their is an issue 
$username = $password = $confirm_password = "";
$username_err = $password_err = $confirm_password_err = "";
 
//Validating and Checking the Data from the form when the submit button is pressed.
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        if($stmt = mysqli_prepare($conn, $sql)){
            //Bind variables to the prepared statement as parameters to avoid SQL Queries getting input
            ...
            if(mysqli_stmt_execute($stmt)){
              ...
              //Hold On to the result to check it is not already taken
    ...
    //Check password is greater than 6 characters and the text input is not empty
    if(empty(trim($_POST["password"]))){
        $password_err = "Please enter a password.";     
    } elseif(strlen(trim($_POST["password"])) < 6){
      //Character Greater thean 6 
        $password_err = "Password must have atleast 6 characters.";
    } else{
        $password = trim($_POST["password"]);
    }
    
    //Check the Repeated/Confirmed password
    ...
    //Check input errors before inserting in database usign SQL
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        
        //Prepare an insert statement as done before for SQL Security Issues.
...
<html>
    <head>
        ...
        <title>BAYKOST - Create an Account</title>
    </head>
    <body>
        ...
        <div id="form">
          ...
          <h1>Create an Account</h1>
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
              <!--UserName with Errors ther ready to be viewable when triggered by php -->
              <label>Username</label>
              <input type="text" name="username" class="form-control <?php echo (!empty($username_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $username; ?>">
              <span class="invalid-feedback"><?php echo $username_err; ?></span>
              <!--Password with Errors ther ready to be viewable when triggered by php -->
              <label>Password</label>
              <input type="password" name="password" class="form-control <?php echo (!empty($password_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $password; ?>">
              <span class="invalid-feedback"><?php echo $password_err; ?></span>
              <!--Repeat/Comnfrim Password with Errors ther ready to be viewable when triggered by php -->
              <label>Repeat Password</label>
              <input type="password" name="confirm_password" class="form-control <?php echo (!empty($confirm_password_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $confirm_password; ?>">
              <span class="invalid-feedback"><?php echo $confirm_password_err; ?></span>
              <!--If there are no errors to echo then they will be blank due to them being intitialized in the php-->
        ...
        <script src="/createAccountValidation.js" async defer></script>
    </body>
</html>